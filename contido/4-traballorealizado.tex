...
Por cada época de adestramento (2500 en total), 10000 puntos son muestreados aleatoriamente do espazo de coordenadas dentro da máscara.
O termo de perda é a 'normalized cross-correlation' entre os valores dos píxeles muestreados na imaxe fixa e os correspondentes da imaxe móbil.
...
Non obstante, para redes de representación implícita como coa que estamos traballando, esta ten unha serie de desventaxas.
...
Outra das vantaxes que ten SIREN é que é unha función suave ou infinitamente diferenciable, é dicir, que admite derivadas de calquera orde.
...
\caption{Inicialización primeira capa}
\end{figure}
...
Unha consecuencia desto é que, xa que os propios pesos da rede representan a deformación, inicialmente a rede comeza cunha deformacion moi similar en todos os casos, que o adestramento deberá corrixir.
...
\subsubsection{Termos de Perda}
\label{subsubsec:Termos de Perda}
...
parece máis apropiado empregar métricas baseadas na similitude como NCC ou SSIM.
...
Debido a que o rexistro de imáxenes deformables é un problema mal formulado (ill-posed problem), 
é común utilizar algún tipo de regularización sobre o DVF para evitar deformacións pouco realistas.
 ...
 como mostras en una cuadrícula de vóxeles, e polo tanto, tan só se poden aproximar gradientes espaciais
...
\begin{itemize}
    ...
    O determinante Xacobiano da transformación (det ∇Φ) nunha localización x é un indicador de estiramento ou compresión local.
    Un determinante Xacobiano negativo ou moi cercano a 0 indica que están a ocurrir dobreces e a transformación non será invertible.
    ...
    O termo de regularización do Xacobiano penaliza os valores do determinante Xacobiano que se desvían de 1, 
    ...
    A matriz de cofactores da matriz do Xacobiano da transformación controla o área, 
    ...
    αl, αa e αν son hiperparámetros que controlan a importancia de cada termo.
...
Para a implementación neste traballo modificaronse todos estos termos para que funcionaran con transfomacións de dúas dimensións en lugar de tres,
 sustituíndo o gradiente de 3 dimensión no Xacobiano por un de dúas, eliminando as derivadas parciais en z en bending energy e o termo de volume no termo hiperelástico.
...
Coa regularización do Xacobiano, ademais do termo de similitude, calcúlanse dúas derivadas (unha por dimensión) para obter o Xacobiano, resultando en 3 pasadas por época.
...
Se os termos de regularización teñen demasiada influencia sobre o termo de perda, a rede fará transformacións moi pequenas para evitar ser penalizada, o que resultará nunha transformación insuficiente.
...
Por exemplo, o regularizador hiperelástico pode considerarse un termo máis xeral que inclúe indirectamente penalizaciones do Xacobiano (ambos penalizan as "dobreces") e de suavidade das transfomacións (como fai o bending pero en menor grado).

\subsubsection{Learning rate e tamaño de lote}
...
Debido á natureza da rede, o tamaño de lote utilizado ten unha relación directa co learning rate, polo que tentaremos determinar a relación óptima entre ambos.

Unha das heurísiticas máis comúns para relacionar o learning rate e o tamaño de lote é a regla de escalado linear \cite{goyal2018accuratelargeminibatchsgd}. 
A regla indica que o learning rate óptimo debe escalarse linearmente cantidade de mostras por lote. 

Unha forma de explicar isto é, xa que con tamaños de lote máis grandes temos unha mellor aproximación do gradiente real, é posible utilizar un learning rate maior sen que a rede diverxa. \cite{kexuefm}

O tamaño de lote nesta rede representa o número de coordenadas amostradas aleatoriamente do espazo de coordenadas dentro da máscara.
...
grazas a iso o mesmo modelo pode ser empregado para imaxes de calquera tamaño, ao contrario dunha CNN tradicional 
...
Finalmente, o proceso de avaliación tivo que implementarse dende 0 para adaptarse ao novo formato de imaxes e datos de referencia.
...
A continuación, comeza o bucle de adestramento, que se repite durante un número predefinido de épocas. En cada iteración dentro dunha época, realízase o seguinte proceso:
\begin{itemize}
    \item \textbf{Mostraxe de Coordenadas:} En lugar de procesar a imaxe enteira en cada paso, selecciónase un subconxunto de puntos do tensor de coordenadas igual ao tamaño de lote. A estratexia de mostraxe é un compoñente crítico que foi obxecto de experimentación neste traballo.
...
\label{subsec:FIRE}
 ...
 Está composto por 134 pares de imaxes de retinas, con un tamaño de 2912 × 2912 píxeles e un \gls{FOV} de 45◦× 45◦.
...
Unha vantaxe de utilizar dous conxuntos de datos diferentes é que cada un deles ten características únicas que permiten avaliar o modelo en diferentes contextos.
...
Pola contra, FIRE é un conxunto de datos real, no cal existen cambios na iluminación, contraste, superposición e demais diferencias visuais, polo que se avalía a capacidade do modelo para realizar rexistros en condicións moito máis adversas.
...
A un tempo de adestramento de 3 minutos por parella, o adestramento completo levaría máis de 6 horas por cada combinación de parámetros, cunha pegada de memoria de arredor de 5 GB de VRAM.
O custo temporal e de memoria dependen de varios factores, sendo os máis relevantes a regularización empregada, a resolución da imaxe, o tamaño do tamaño de lote e a función de activación.
...
Nesta fase avaliouse a métrica de perda, a resolución da imaxe, a regularización empregada e o tamaño do tamaño de lote.

Baseándose nos resultados da primeira fase, realizouse unha experimentación máis exhaustiva para tentar mellorar o rendemento do rexistro, centrándose na estratexia de mostraxe, a inicialización dos pesos da rede e un axuste dinámico do tamaño de lote.
...
Introduciuse unha estratexia de mostraxe uniforme, onde se selecciona un número fixo de puntos en cada imaxe, asegurando que están distribuídos uniformemente por toda a imaxe. É unha estratexia similar á mostraxe aleatoria, pero garantindo que se cubre a maior parte posible da imaxe. Isto é relevante en experimentos con tamaños de lote pequenos, onde unha mostraxe aleatoria non ten por que cubrir todas as zonas da imaxe. Para implementalo, empregouse unha distribución baseada na grella de Fibonacci (Fibonacci lattice), que permite repartir os puntos de maneira uniforme sobre a superficie circular da retina. A posición de cada punto calcúlase en coordenadas polares, asignando a cada punto un raio proporcional á raíz cadrada do seu índice dividido polo número total de puntos, e un ángulo proporcional ao índice multiplicado por $2\pi$ e dividido polo cadrado do número áureo ($\varphi^2$):
...
Implementouse o axuste dinámico do tamaño do lote, que consiste en aumentar a cantidade de mostras tomadas pola rede ao longo do adestramento. Para levar a cabo esta estratexia, divídense as épocas en diferentes fases, onde cada fase fai uso dun tamaño de lote diferente, comezando normalmente con tamaños máis pequenos e aumentándoos progresivamente.
...
Esta é unha xeralización da distancia euclidiana aplicada a matrices, onde as imaxes con transformacións máis grandes considéranse máis difíciles.
...
No caso deste traballo, a avaliación cualitativa cobra gran importancia, xa que na cuantitativa só se está a comparar sobre un número reducido de puntos en cada parexa de imaxes.
...
Na figura \ref{fig:visex} pódense observar algúns exemplos destas.
